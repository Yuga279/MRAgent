...
- [server/](server/) — Express + `ws` backend ([server/server.js](server/server.js)) with three surfaces:
  ...
  Support-agent domain logic lives in [server/src/support.js](server/src/support.js) (knowledge loading, `get_order_status`/`place_order` handlers, function schemas, prompt, reply sanitizer) so it is unit-testable without starting the server; `MRAGENT_DATA_DIR` overrides the data directory for tests. `place_order` has three server-side guards (LLMs proved too eager, placing orders on garbled voice input): the model must pass `customer_confirmed=true`, the customer's actual last utterance must match an affirmation pattern (`looksLikePurchaseConsent`), and back-to-back duplicates of the same product within 3 minutes are rejected. Both function-call paths (free-mode chat loops and the Deepgram relay's `handleFunctionCallRequest`) thread `context.lastUserMessage` into handlers — the relay tracks it from user `ConversationText` events.
...
### Knowledge base: Pinecone RAG (optional)

When `PINECONE_API_KEY` is set, the knowledge base moves from the prompt into a Pinecone vector index ([server/src/rag.js](server/src/rag.js), REST API with Pinecone integrated embeddings `llama-text-embed-v2` — no separate embedding key needed). Knowledge then reaches the model differently per path — `buildSupportPrompt({retrievedKnowledge})` has three modes: `/api/chat` retrieves top-3 chunks up front and injects them (`retrieved`); the Deepgram agent gets a `search_knowledge` function in `SUPPORT_FUNCTIONS` since its prompt is fixed at session start (`tool`); without the key knowledge.md is inlined as before (`inline`). `SUPPORT_FUNCTIONS` and the mode are decided at module load, so changing the key requires a restart. Ingest/re-ingest after editing [server/data/knowledge.md](server/data/knowledge.md) with `cd server && npm run ingest-knowledge` (chunks by `##` section, auto-creates the index `mragent-knowledge`, override via `PINECONE_INDEX`). Function handlers may be async — the agent relay's `handleFunctionCallRequest` `await`s them.
